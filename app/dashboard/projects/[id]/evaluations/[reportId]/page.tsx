...
export default async function ProjectEvaluationResultPage({
  params
}: {
  ...
  const report = (await listReportsByProject(project.id)).find((item) => item.id === reportId);
    ...
    <div className="space-y-6">
      <Card className="rounded-[30px]">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-sm font-medium text-slate-500">Saved evaluation result</div>
          ...
          <div className="rounded-[24px] border border-slate-200 bg-slate-50 px-5 py-4 text-right">
            ...
            <div className="mt-2 text-4xl font-semibold tracking-[-0.04em] text-slate-950">{report.overallScore}</div>
            <div className="mt-2 text-sm text-slate-600">{getEvaluationBand(report.overallScore)}</div>
            <div className="mt-2 text-xs text-slate-500">{formatTimestamp(report.createdAt)}</div>
          </div>
        </div>
      ...
      <Card className="rounded-[30px]">
        <h2 className="text-xl font-semibold text-slate-950">Overall summary</h2>
        <p className="mt-4 text-sm leading-7 text-slate-700">{report.jsonReport.overallSummary}</p>
      </Card>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card className="rounded-[30px]">
          ...
          <ul className="mt-4 space-y-2 text-sm leading-7 text-slate-700">
            {report.jsonReport.strengths.map((item) => (
              <li key={item}>• {item}</li>
            ))}
          </ul>
        </Card>
        ...
        <Card className="rounded-[30px]">
          <h2 className="text-xl font-semibold text-slate-950">Priority improvements</h2>
          <ul className="mt-4 space-y-2 text-sm leading-7 text-slate-700">
            {report.jsonReport.priorityImprovements.map((item) => (
              <li key={item}>• {item}</li>
            ))}
          </ul>
        </Card>
        ...
        <Card className="rounded-[30px]">
          <h2 className="text-xl font-semibold text-slate-950">Revision checklist</h2>
          <ul className="mt-4 space-y-2 text-sm leading-7 text-slate-700">
            {report.jsonReport.revisionChecklist.map((item) => (
              <li key={item}>• {item}</li>
            ))}
          </ul>
        </Card>
        ...
        <Card className="rounded-[30px]">
          <h2 className="text-xl font-semibold text-slate-950">Citation feedback</h2>
          <ul className="mt-4 space-y-2 text-sm leading-7 text-slate-700">
            {report.jsonReport.citationFeedback.map((item) => (
              <li key={item}>• {item}</li>
            ))}
          </ul>
        </Card>
