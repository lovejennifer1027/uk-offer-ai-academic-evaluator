  ...
  return (
    <PageShell>
      <section className="hero-shell relative overflow-hidden pb-24 pt-14 md:pb-28 md:pt-20">
        <div className="page-container grid gap-10 xl:grid-cols-[1.02fr_0.98fr] xl:items-center">
          <div className="relative z-10">
            ...
            <h1 className="mt-7 max-w-5xl text-5xl leading-[1.02] text-[var(--navy)] md:text-6xl xl:text-7xl">
              像真正教育咨询产品一样的 AI 论文评估体验。
            </h1>
            <p className="mt-7 max-w-3xl text-lg leading-9 text-[var(--muted)]">
              {BRAND_NAME} 面向中文用户打造，把论文初评、老师要求和正式报告整合进一个更轻、更高级、也更适合商业展示的学术产品界面。
            </p>
            ...
            <div className="mt-8 flex flex-wrap gap-3 text-sm">
              ...
              <span className="quiet-badge">结构化输出更稳定</span>
            </div>

            <div className="mt-11 grid gap-4 sm:grid-cols-3">
              {heroMetrics.map((item) => (
                <div key={item.value} className="metric-chip">
          ...
          <div className="card-surface relative z-10 overflow-hidden rounded-[44px] p-7 md:p-10">
            <div className="absolute inset-x-0 top-0 h-32 bg-[linear-gradient(180deg,rgba(160,189,229,0.16),transparent)]" />
            <div className="relative">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <p className="section-eyebrow text-sm font-semibold text-[var(--gold)]">产品预览</p>
                  <h2 className="mt-4 text-2xl text-[var(--navy)] md:text-3xl">
                    更像顾问桌面，不像学生作业页。
                  </h2>
                </div>
                <span className="rounded-full border border-[rgba(141,139,198,0.18)] bg-[rgba(141,139,198,0.1)] px-4 py-2 text-xs font-semibold text-[var(--navy)]">
                  形成性反馈
                </span>
              </div>
              ...
              <div className="mt-7 space-y-5">
                <article className="rounded-[32px] border border-[rgba(59,76,107,0.08)] bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(240,245,252,0.96))] p-6 text-[var(--navy)] shadow-[0_18px_42px_rgba(67,84,120,0.08)]">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <p className="text-sm font-semibold">学生论文</p>
                    <span className="rounded-full border border-[rgba(59,76,107,0.08)] bg-white px-3 py-1 text-xs text-[var(--muted)]">
                      已准备评估
                    </span>
                  </div>
                  <p className="mt-4 text-lg leading-8 text-[var(--muted)]">
                    论文正文、论点结构与文献引用被放进同一个工作流，不需要切换多个页面。
                  </p>
                </article>

                <div className="grid gap-5 lg:grid-cols-[1.08fr_0.92fr]">
                  <article className="surface-inset rounded-[32px] p-6">
                    <p className="text-sm font-semibold text-[var(--navy)]">老师要求 / 评分标准</p>
                    <p className="mt-3 text-sm leading-7 text-[var(--muted)]">
                      当老师标准明确时，系统优先按老师要求评分；若要求模糊，再回退到英国高校通用学术写作标准。
                    </p>
                    <div className="mt-5 flex flex-wrap gap-2 text-xs font-semibold text-[var(--navy)]">
                      <span className="rounded-full bg-white px-3 py-2">评分要求优先级更高</span>
                      <span className="rounded-full bg-white px-3 py-2">形成性反馈</span>
                      <span className="rounded-full bg-white px-3 py-2">结构化结果</span>
                    </div>
                  </article>
        ...
        <div className="grid gap-10 xl:grid-cols-[0.8fr_1.2fr] xl:items-start">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-8 text-[var(--muted)]">
              参考你给的方向，这一版把“马上开始使用”放在更前面。用户不需要先看很多解释，就能理解输入什么、系统会返回什么。
            </p>
          </div>

          <div className="grid gap-5 md:grid-cols-3">
            {processSteps.map((step) => (
              <article key={step.step} className="card-surface rounded-[36px] p-7">
                <p className="text-sm font-semibold text-[var(--gold)]">{step.step}</p>
      ...
      <section className="page-container py-8 md:py-12">
        <div className="grid gap-9 xl:grid-cols-[0.86fr_1.14fr]">
          <div className="card-surface rounded-[40px] p-8 md:p-10">
            ...
            <p className="mt-5 text-base leading-8 text-[var(--muted)]">
              我们保留正式、克制的学术语气，但页面本身更接近成熟产品界面：信息层次更简单，卡片更干净，重点更聚焦。
            </p>
          </div>
          ...
          <div className="grid gap-9 xl:grid-cols-[0.92fr_1.08fr] xl:items-start">
            <div>
              ...
              <p className="mt-5 max-w-3xl text-base leading-8 text-white/72">
                如果你喜欢 easyessay 那种更像在线产品的感觉，这一版已经把首页和工作台收拢到“更直接进入任务、更少展示口号”的方向，同时保留 UK Offer 更高端、更可信的定位。
              </p>
            </div>
      ...
      <section className="page-container pb-24">
        <div className="card-surface rounded-[42px] p-9 md:p-11">
