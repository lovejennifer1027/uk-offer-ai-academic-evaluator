    ...
    <div className="grid gap-6 xl:grid-cols-[1.04fr_0.96fr]">
      <div className="space-y-6">
        <Card className="hero-shell overflow-hidden rounded-[36px] p-0">
          <div className="relative grid gap-6 px-7 py-7 lg:grid-cols-[1.1fr_0.9fr] lg:px-8">
            <div>
              <div className="eyebrow-pill text-xs font-semibold">School-aware evaluation engine</div>
              <h2 className="mt-5 text-3xl font-semibold tracking-[-0.05em] text-slate-950">先选学校，再用学校资料评估这份作业。</h2>
              <p className="mt-4 max-w-xl text-sm leading-7 text-slate-600">
                这个工作台会优先调用你所选学校的课程要求、rubric 和项目知识库，再结合论文草稿与评估标准生成分项清晰的可视化报告。
              </p>
              <div className="mt-6 grid gap-3 sm:grid-cols-3">
                {[
                  { icon: Building2, label: "当前项目", value: projectTitle },
                  { icon: GraduationCap, label: "模块", value: moduleCode },
                  { icon: Database, label: "知识策略", value: `${schoolProfile.shortName} 优先` }
                ].map((item) => (
                  <div key={item.label} className="rounded-[24px] border border-white/70 bg-white/78 px-4 py-4 shadow-[0_16px_32px_rgba(67,84,120,0.06)] backdrop-blur-xl">
                    <item.icon className="h-4 w-4 text-slate-500" />
                    <div className="mt-3 text-xs font-medium uppercase tracking-[0.18em] text-slate-400">{item.label}</div>
            ...
            <div className="rounded-[28px] border border-white/75 bg-white/82 p-5 shadow-[0_18px_38px_rgba(67,84,120,0.06)] backdrop-blur-xl">
              <div className="text-sm font-semibold text-slate-900">学校知识模式</div>
              <div className="mt-3">
                <Select value={selectedSchool} onChange={(event) => setSelectedSchool(event.target.value)} className="rounded-[20px] border-white bg-white/80">
              ...
              <p className="mt-4 text-sm leading-7 text-slate-600">{schoolProfile.supportNote}</p>
        ...
        <div className="grid gap-6">
          <Card className="rounded-[34px] p-0">
            <div className="border-b border-slate-100 px-7 py-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-sm font-semibold text-slate-900">上方：论文内容</div>
                  <p className="mt-2 text-sm leading-7 text-slate-600">
                    粘贴论文正文，或直接把论文文件拖进来。系统会先把内容放进当前评估，再同步到项目知识库。
                  </p>
                </div>
            ...
            <div className="px-7 py-6">
              <Textarea
                ...
                className="min-h-[300px] rounded-[26px] border-white bg-white/84 px-5 py-4 text-sm leading-7 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]"
          ...
          <Card className="rounded-[34px] p-0">
            <div className="border-b border-slate-100 px-7 py-6">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-sm font-semibold text-slate-900">下方：学校要求与评分标准</div>
                  <p className="mt-2 text-sm leading-7 text-slate-600">
            ...
            <div className="grid gap-6 px-7 py-6 lg:grid-cols-[1fr_0.38fr]">
              <div>
                <Textarea
                  value={rubricText}
                  onChange={(event) => setRubricText(event.target.value)}
                  className="min-h-[220px] rounded-[26px] border-white bg-white/84 px-5 py-4 text-sm leading-7 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]"
              ...
              <div className="rounded-[28px] border border-slate-200 bg-slate-50/90 p-5">
                <div className="text-sm font-semibold text-slate-900">评估参数</div>
                <div className="mt-4 space-y-4">
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Target level</div>
                    <Input
                      value={targetLevel}
                      onChange={(event) => setTargetLevel(event.target.value)}
                      className="mt-2 rounded-[20px] border-white bg-white"
                    />
                  </div>
                  <div>
                    <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Citation style</div>
                    <Select
                      value={citationStyle}
                  ...
                  <div className="rounded-[20px] bg-white px-4 py-4 text-sm leading-7 text-slate-600">
                    评估会优先使用 <span className="font-semibold text-slate-900">{schoolProfile.shortName}</span> 的知识上下文，再结合项目里已上传的学校资料做检索支持。
                  </div>
                </div>
              </div>
            </div>
